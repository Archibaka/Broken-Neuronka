...
int main() {
    ...
    long double lambda = 58055 * 69;
    long double eta = 0.00037;
    u(x, paths, SaveAs, st, 2, eta, 1, 100, lambda, 1);
    chrono::duration<long double> z = system_clock::now().time_since_epoch();
    cout << z.count();
    //shuffle(begin(paths), end(paths), default_random_engine{});
    /*shuffle(std::begin(paths), end(paths), default_random_engine{});
     vector<string> dat;
     for (string c : paths) {
         ifstream in;
         string line;
     ...
     std::shuffle(std::begin(dat), std::end(dat), std::default_random_engine{});
     int counter = 0;
     string base = "C:\\Users\\mirop\\Desktop\\Lapa1337\\newdat\\idk";
     string ed = ".txt";
     int hm = round(dat.size() / paths.size());
     ofstream out;
     out.open(base + std::to_string(counter) + ed);
